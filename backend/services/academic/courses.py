...
class CourseService:
    ...
    async def create(self, course_in: CourseCreate, tenant_id: UUID) -> Course:
        course = Course(
            name=course_in.name,
    ...
    async def get(self, course_id: UUID, tenant_id: UUID) -> Optional[Course]:
        # Get the course with tenant filtering and eager load relationships
        course_stmt = select(Course).options(
            selectinload(Course.lecturer),
        ).where(
            Course.id == course_id,
    ...
    async def list(
        ...
        base_stmt = select(Course).options(
            selectinload(Course.lecturer),
        )
        if tenant_id:
            base_stmt = base_stmt.where(Course.tenant_id == tenant_id)
        if lecturer_id:
            base_stmt = base_stmt.where(Course.lecturer_id == lecturer_id)

        # Get total count
        count_stmt = select(func.count()).select_from(Course)
        if tenant_id:
            count_stmt = count_stmt.where(Course.tenant_id == tenant_id)
        if lecturer_id:
            count_stmt = count_stmt.where(Course.lecturer_id == lecturer_id)
        total_result = await self.db.execute(count_stmt)
        total = int(total_result.scalar_one())

        # Apply pagination
        stmt = base_stmt.order_by(Course.created_at.desc()).offset(offset).limit(limit)
    ...
    async def count(self, tenant_id: Optional[UUID] = None, lecturer_id: Optional[UUID] = None) -> int:
        ...
        if lecturer_id:
            stmt = stmt.where(Course.lecturer_id == lecturer_id)
        res = await self.db.execute(stmt)
